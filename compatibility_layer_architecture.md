# Spigot-Airforce Compatibility Layer Architecture

## 1. Introduction

This document outlines the proposed architecture for a compatibility layer designed to allow existing Spigot plugins to run on the Airforce server software with minimal modification. Airforce's multi-region architecture introduces significant changes to threading and data access, which the traditional Spigot API was not designed for. This layer aims to bridge that gap.

The previous analysis (`spigot_api_thread_safety_analysis.md`) identified numerous areas where Spigot API calls could lead to thread-safety issues in Airforce. This architecture directly addresses those concerns.

## 2. Core Principles

The design of the compatibility layer is guided by the following core principles:

*   **Transparency (Maximize):** For plugin developers, interaction with the Bukkit API should feel as close to vanilla Spigot as possible. The layer should ideally hide the complexity of Airforce's regionalization unless absolutely necessary.
*   **Safety (Prioritize):** The primary goal is to prevent thread-safety issues like data races, deadlocks, and stale reads. The layer must ensure that API calls are executed in a way that is compatible with Airforce's threading model.
...
*   **Delegation (Prefer):** Operations that modify region-specific data (e.g., block changes, entity updates) must be delegated to the appropriate Airforce region thread for execution.
...
*   **Mechanism:** A combination approach is likely best:
    ...
    *   **Wrapper Objects / Facades:** For Bukkit interfaces or non-final classes, the servercould provide custom implementations that wrap Airforce's native objects. For example, when a plugin calls `player.getWorld()`, it might receive a `WorldWrapper` that internally delegates calls to the actual Airforce world object, adding necessary safety checks and thread delegation.
...
*   **Key Responsibilities:**
    ...
    *   **Context Identification:** Determine the context of the call:
        *   Is it from a plugin thread, a global Airforce thread, or a specific region thread?
    ...
    *   **Result Marshalling:** When a delegated task completes, ensure its result is safely returned to the plugin, potentially wrapping returned objects (e.g., Airforce entities) in Bukkit API proxies.
    *   **API Behavior Replication:** For methods that Airforce might handle differently (e.g., an API that used to block might now be asynchronous), the shim layer may need to simulate the original behavior if plugins rely heavily on it, or clearly document the change.

---

### 3.2. Region Task Delegator / Scheduler

*   **Purpose:** This component is responsible for taking a piece of work (e.g., a lambda, a `Runnable`, or a `Callable` representing an API call) identified by the Shim Layer and ensuring it executes on the correct Airforce region thread. It also handles cases where an operation might need to execute on a global server thread.
...
*   **Interaction with Airforce's Native Schedulers:**
    *   This delegator will be a layer on top of Airforce's own scheduling mechanisms.
    *   When `runOnRegion(Location, ...)` is called, it will use Airforce's API to determine the region associated with the `Location`.
    *   It will then use Airforce's equivalent of `region.getScheduler().execute(task)` or `region.getScheduler().call(task)` to schedule the work.
    *   It needs to handle the `Future` object returned by Airforce's scheduler and make it available to the compatibility layer, which might, in turn, need to block and wait for this `Future` if the original Spigot call was synchronous. This is a critical point for maintaining API call semantics.
    *   For operations that don't have a specific location (e.g., some `Server` methods), it might default to a global Airforce thread or a designated primary region thread if applicable.

*   **Key Responsibilities:**
    *   Determining the target execution context (specific region thread, global thread).
    *   Scheduling the task using Airforce's native scheduling primitives.
...
*   **Purpose:** When a plugin obtains a Bukkit object (like `Player`, `Entity`, `World`, `Block`, `ItemStack`), it often receives a proxy or facade object instead of a direct reference to Airforce's internal representation. These proxies ensure that interactions with the object are thread-safe and region-aware.
...
*   **Behavior for Read/Write Operations:**
    *   **Read Operations (e.g., `player.getName()`, `block.getType()`, `itemStack.getAmount()`):**
        ...
        *   If the data is region-specific and the calling thread is the owning region's thread, the call might be allowed to pass through to the underlying Airforce object.
...
*   **Cross-Region Identification:**
    *   Proxies for objects like `Entity` or `Player` must retain information about which Airforce region currently "owns" or ticks that object. This is crucial for the "Region Task Delegator."
    *   For `Block` or `Location` proxies, the world and coordinates directly provide the means to identify the region.
    *   `World` proxies would inherently know which world they represent, and Airforce would map this to its regional data structures.
...
*   **Purpose:** Manages the firing and handling of Bukkit events in a way that is compatible with Airforce's threading model and Spigot plugin expectations.

*   **Event Object Handling:**
    *   **Immutability/Copies:** When Airforce fires an event that will be passed to Spigot plugins, the Event Bus Adapter should ensure that the event object itself, and any mutable objects it contains (e.g., `List<Entity>`, `ItemStack`), are immutable snapshots or defensive copies. This prevents plugin handlers running on different threads (or a Spigot plugin handler interfering with Airforce's internal event processing) from causing issues with shared mutable state.
...
*   **Ensuring Correct Thread for Handlers:**
    *   **Airforce-Originated Events:** Airforce will likely fire events on the region thread where they occur (e.g., `BlockBreakEvent` on the region thread for that block's location). The Event Bus Adapter will pass these to Spigot plugin listeners. Spigot listeners will thus execute on that Airforce region thread. This is a fundamental shift from traditional Spigot where most such events run on the main server thread. Plugins need to be coded defensively (the compatibility layer cannot magically make non-thread-safe handlers safe, but it can provide safe data).
    *   **Global Events:** Events that are inherently global (e.g., `ServerListPingEvent`) might be fired by Airforce on a main/global thread. Spigot handlers would then also run on this thread.
...
*   **Handling `PluginManager.callEvent()` from Plugins:**
    *   If a Spigot plugin calls `Bukkit.getPluginManager().callEvent(someEvent)`:
        ...
        *   **If called from an Airforce region thread:** The event handlers will be executed synchronously on that *same region thread*. This is consistent with how Airforce itself would fire a region-specific event.
        *   **If called from an Airforce global thread:** Handlers run on that global thread.
...
*   **Purpose:** To provide a `org.bukkit.scheduler.BukkitScheduler` implementation that behaves predictably for plugins while ensuring tasks execute on appropriate Airforce threads. This component will replace the standard Bukkit/Spigot scheduler.
...
*   **Interpreting Sync and Async Tasks:**
    *   **`runTask(Plugin, Runnable)` (and its variants `runTaskLater`, `runTaskTimer`):**
        ...
        *   In Airforce, the adapter needs to determine the "context" for this sync task if possible.
        *   **Option 1 (Simplest):** All such tasks are scheduled on a designated "Global Main Thread" in Airforce. This is safest for compatibility but means region-specific operations within these tasks will still incur cross-thread overhead via API interceptors.
        *   **Option 2 (More Advanced):** Introduce new Airforce-aware API extensions that plugins *could* use, e.g., `runTaskAt(Plugin, Location, Runnable)`. For the standard `runTask`, if the plugin code is currently executing on an Airforce region thread (e.g., within an event handler), the adapter could schedule the sync task onto that *same region thread's next tick*. If called from an async task or global thread, it defaults to the Global Main Thread.
    ...
    *   **`runTaskAsynchronously(Plugin, Runnable)` (and its variants):**
        ...
        *   The BukkitScheduler Adapter can delegate these to a generic Airforce worker thread pool.
...
*   **Region-Aware Task Execution:**
    ...
    *   If Airforce offers its own explicit region-bound schedulers (e.g., `entity.getScheduler().runTask()`), the compatibility layer should encourage advanced plugins to use these directly for better performance where possible, perhaps by exposing them through the Proxies/Facades (e.g., `((PlayerProxy)player).getAirforceScheduler().runTask(...)`).

*   **Task Cancellation (`BukkitTask.cancel()`):**
    *   The adapter needs to manage `BukkitTask` objects and allow their cancellation. This requires thread-safe tracking of scheduled tasks and interaction with Airforce's underlying scheduler to cancel the corresponding native task.
...
1.  **Plugin Code:** `myBlock.setType(Material.STONE);`
    ...
    *   Assume this code is running on a thread that is *not* the Airforce region thread for `myBlock`'s location (e.g., a command handler thread, or a BukkitRunnable scheduled to the global main thread).
...
3.  **Region Task Delegator / Scheduler:**
    ...
    *   The Delegator uses `blockLocation` to identify the correct Airforce region (e.g., Region X).
    *   It schedules a task with Airforce's scheduler for Region X to execute the lambda: `airforceRegionX.getScheduler().execute(() -> { underlyingAirforceBlock.setType(Material.STONE); });`
...
4.  **Execution on Region Thread:**
    *   On Region X's next tick, Airforce's scheduler executes the task.
    *   The actual Airforce block object's `setType` method is called. This is now happening on the correct, region-safe thread.
    *   Any internal Airforce events related to block changes are fired on Region X's thread.
...
3.  **Region Task Delegator / Scheduler:**
    ...
    *   It schedules a task with Airforce's scheduler for Region A: `airforceRegionA.getScheduler().execute(() -> { underlyingAirforcePlayer.teleport(targetLocation); });`
        *   **Note:** The actual teleport logic within Airforce might be complex. If `targetLocation` is in Region B, Airforce's `underlyingAirforcePlayer.teleport()` will handle the cross-region handoff (removing from Region A's ticking, adding to Region B's ticking, updating player data structures, etc.). This internal Airforce complexity is ideally hidden from the Spigot plugin.
...
4.  **Execution on Region Thread(s):**
    ...
    *   Airforce's internal teleport logic takes over. This might involve coordination with `targetLocation`'s region (Region B) and potentially a global player management system. The entity's data is safely transferred.
    *   Events like `PlayerTeleportEvent` would be fired. The Event Bus Adapter ensures these are correctly processed (see component 3.4). If the teleport event is cancelled by a Spigot plugin, this cancellation needs to be communicated back to the Airforce teleport logic.

5.  **Return / Unblocking:**
    *   Once Airforce confirms the teleport is complete (or has failed but processed), the task on Region A (or wherever the final responsibility lies) completes.
...
*   **Interaction with Airforce-Native Plugins:**
    *   How does this layer interact if a "Spigot" plugin tries to communicate directly with an "Airforce-native" plugin that is already region-aware? This is likely outside the direct scope of initial compatibility but a long-term consideration.
