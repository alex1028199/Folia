...
/**
 * Interface for delegating tasks to be run on the appropriate Folia region scheduler
 * or a global thread.
 */
public interface RegionTaskDelegator {
     ...
     * @param task   The task to execute.
     */
    void scheduleForInventoryHolder(io.papermc.folia.compat.wrappers.inventory.InventoryHolderWrapper holder, Runnable task);
     ...
     * @return A Future representing the pending result of the task.
     */
    <T> Future<T> callForInventoryHolder(io.papermc.folia.compat.wrappers.inventory.InventoryHolderWrapper holder, Callable<T> task);


    // Placeholder for getting a region identifier. In a real implementation,
    // this would interact with Folia's region management.
